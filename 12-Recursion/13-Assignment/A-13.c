...
int hcf(int**,int,int,int,int[],int);

int main() {
    printf("\n\t ***** Finding HCF of the numbers using Recursion ***** \n");
    ...
    n = hcf(arr,i-1,common_factor_index,common_factor_length+1,common_factor,0);
...
int hcf(int **arr,int n,int i,int j,int common_factor[],int t) {
    if (j == 0)    return t;
